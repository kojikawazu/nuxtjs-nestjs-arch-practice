...
- backend-layered: 単体 74 / e2e 35 / IT 4（JWT 秘密鍵は起動時検証・リフレッシュ消費は原子的・画像添付は失敗時に補償削除・入力検証は zod・DryRun 廃止済み・検証失敗は 422 + errors）
- backend-clean: 単体 119 / e2e 35 / IT 4（JWT 秘密鍵は起動時検証・リフレッシュ消費は原子的・画像添付は失敗時に補償削除・同一 e2e シナリオ・tasks 読み取りは CQRS 分離・application/presentation 細分化・auth/users もクリーン化・入力検証は zod・検証失敗は 422 + errors）
- backend-onion: 単体 122 / e2e 35 / IT 4（JWT 秘密鍵は起動時検証・リフレッシュ消費は原子的・画像添付は失敗時に補償削除・同一 e2e シナリオ・tasks 読み取りは CQRS 分離・auth/users もクリーン化・入力検証は zod・検証失敗は 422 + errors・application は presentation 非依存）
