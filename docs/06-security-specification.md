...
- **ローテーションの消費は 1 回だけ成功する。** 照合（`findMatch`）と失効（`consumeById`）は別クエリなので、同じトークンで並行にリフレッシュすると両方が同じ行を見てしまう。`DELETE ... WHERE id = ?` の**影響行数**を排他判定に使い、行を消せた呼び出しだけが新しいトークンペアを発行する（負けた側は 401）。
  - 追加のトランザクションやロックは持ち込まない。DB が行ロックで直列化するため、既にある DELETE 1 本で「勝者が 1 つだけ」を保証できる。
  - これが崩れると **1 本のリフレッシュトークンから 2 組のトークンペアが生まれ**、ローテーションによる再利用検知の前提が壊れる。
  - 3 版とも同じ挙動（layered は `Repository.delete()` の `affected` をその場で判定、clean / onion は `RefreshTokenRepository.consumeById()` が契約として `boolean` を返す）。
...
- メール: DTO で **255 文字以内**に制限する（登録・ログインの両方）。DB カラム `users.email` の `varchar(255)` と同値。
  - 上限が無いと超過分が INSERT まで到達し、MySQL のカラム長エラーが未分類例外として **500** になる（入力起因の失敗が障害に見える）。
  - パスワードと同じく**ログイン側にも同じ上限を課す**（片方だけ開けておかない）。判定は zod の `.max(MAX_EMAIL_LENGTH)` で、超過は 422。
