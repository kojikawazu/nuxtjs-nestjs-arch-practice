...
| Validator | 業務ルール（開始≤終了・所有権・メール重複）を検証し、検証済みドメインを返す application 層のクラス。保存はせず、UseCase がこれを通してから保存する。 | [`create.validator.ts`](../apps/backend-onion/src/modules/tasks/application/validators/create.validator.ts) |
