...
> **例外**: タスク新規作成の確認画面のみ、方式差そのものを比較対象とするため実装を分ける（spa = 同一ページ内 step 切替 / ssr = 独立ルート `/tasks/new/confirm` をサーバ描画）。`data-testid` は共通に保ち、共通の E2E シナリオ（`task-flow.spec.ts`）は両方で通す。方式差の検証は各版固有の spec（`ssr-confirm.spec.ts` 等）に切り出す。
