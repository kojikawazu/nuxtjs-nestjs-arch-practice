...
import {
  USER_REPOSITORY,
  type UserRepository,
} from '../../src/api/users/application/ports/user-repository.port';
import { EmailAlreadyRegisteredError } from '../../src/api/users/domain/errors/user.errors';
...
describe('登録の一意制約・メール長 IT（MySQL コンテナ）', () => {
  ...
  describe('同一メールの重複登録', () => {
    ...
    // Repository を直接呼ぶと findByEmail の事前チェックを通らないため、
    // 並行登録で負けた側と同じ経路（DB の一意制約違反）を必ず通る。翻訳が無ければここで
    // QueryFailedError のまま抜け、HTTP では 500 になる。
    it('準正常系: 事前チェックを経ない重複 INSERT も EmailAlreadyRegisteredError になる', async () => {
      const input = { email: 'dup@example.com', passwordHash: 'hashed', displayName: 'Dup' };
      await users.create(input);

      await expect(users.create(input)).rejects.toBeInstanceOf(EmailAlreadyRegisteredError);
    });
