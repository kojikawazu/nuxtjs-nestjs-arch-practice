...
describe('TypeOrmRefreshTokenRepository（ハッシュ保管・照合）', () => {
  ...
  it('正常系: consumeById は行を消せたら true（この呼び出しが回転の勝者）', async () => {
    repo.delete.mockResolvedValue({ affected: 1 });

    expect(await sut.consumeById('rt-1')).toBe(true);
    expect(repo.delete).toHaveBeenCalledWith({ id: 'rt-1' });
  });

  it('準正常系: consumeById は既に消えていたら false（並行リフレッシュに負けた）', async () => {
    repo.delete.mockResolvedValue({ affected: 0 });

    expect(await sut.consumeById('rt-1')).toBe(false);
  });

  it('異常系: ドライバが affected を返さない場合は false 扱いにする（勝者を重複させない）', async () => {
