...
describe('UpdateTaskValidator（検証のみ・保存しない）', () => {
  ...
  it('正常系: 更新を適用した Task を返す（保存はしない）', async () => {
    ...
    const task = await validator.execute(toUpdateTaskInput(USER, 'task-1', { status: 'done' }));
  ...
  it('異常系: 存在しないタスクは TaskNotFoundError', async () => {
    ...
    await expect(
      validator.execute(toUpdateTaskInput(USER, 'missing', { title: 'x' })),
  ...
  it('準正常系: 他人のタスクは TaskAccessDeniedError', async () => {
    ...
    await expect(
      validator.execute(toUpdateTaskInput(USER, 'task-1', { title: 'x' })),
  ...
  it('異常系: マージ後に終了が開始より前なら InvalidDateRangeError', async () => {
    ...
    await expect(
      validator.execute(toUpdateTaskInput(USER, 'task-1', { endDate: '2026-01-05T00:00:00.000Z' })),
