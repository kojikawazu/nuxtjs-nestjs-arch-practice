...
/**
 * タスク参照（読み取り）専用の契約（CQRS の Query 側）。
 *
 * オニオンでは契約をドメイン中核が所有するため、この interface も domain 層に置く
 * （clean は application/ports に置く＝配置上の対比点）。
 * 書き込み側の {@link TaskRepository} と異なりドメイン Task を返さず **契約 Task を直接返す**。
