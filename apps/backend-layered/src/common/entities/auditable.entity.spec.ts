...
describe('AuditableEntity（監査列の集約）', () => {
  it('正常系: createdAt / updatedAt を専用デコレーターで宣言している', () => {
    const modesByProperty = Object.fromEntries(
      columnsOf(AuditableEntity).map((column) => [column.propertyName, column.mode]),
    );
  ...
  ])('正常系: %s は AuditableEntity を継承する', (_name, entity) => {
    expect(entity.prototype).toBeInstanceOf(AuditableEntity);
  });
  ...
  it('準正常系: RefreshTokenEntity は継承せず createdAt のみを持つ', () => {
    expect(RefreshTokenEntity.prototype).not.toBeInstanceOf(AuditableEntity);

    const modesByProperty = Object.fromEntries(
      columnsOf(RefreshTokenEntity)
        .filter((column) => auditColumnModes.includes(column.mode))
  ...
  it('準正常系: 監査列を宣言するのはベースクラスだけ（他クラスに散っていない）', () => {
    ...
    expect(new Set(declaringTargets)).toEqual(new Set([AuditableEntity, RefreshTokenEntity]));
  });
});
